...
%    TODO add more details maybe????
    ...
    A container orchestrator is a tool that managed the entire life cycle of all the microservice contaienrs within our system.
    ...
    Managing resource allocation among existing containers to ensure each container gets the right amount of CPI memory and stroage to work properly.

    Monitoring the health of containers self healing, which allows automatic deployment of new contianres to maintain a healthy number of instances of each microservice.
    ...
    \paragraph{Motivations}
    These patterns allow us to  to architect and deploy a highly scalable system for different domain and use-cases.
    This will enable a business to handle billions of requests per day, process petabytes of data and most importantly and do this in a cost effective way in a cloud environment.
    ...
    **** For these situations, there is another algorithm which is called least connection
    ...
    Another cloud computing feature that typically goes along with load balancing is a feature that is called auto of scaling
    ...
    -- Then once we have the individual chunks separated, we need to select a single frame from each chunk to be the thumbnail
    ...
    - The last important consideration is that this pattern is not a good fit for situation where all the operations within the pipeline need to be performed as part of a transaction
