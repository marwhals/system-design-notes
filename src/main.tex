...
%    TODO add more details maybe????
    ...
    **** For these situations, there is another algorithm which is called least connection
    ...
    Then on the other hand, we have a group of workers that repsond to those senders reqeusts.
    ...
    Unlike the load balancing pattern, where each reqeust from the sender is dispatched to only one worker, in the scatter gather pattern, the dispatch error allow sthe request to all the workers.
    ...
    Another difference between the load balancing pattern and the scatter getter pattern is that unliske in the load balancing pattern where the workers are actualy different.
    Those workers can either be instantces of the saem appliication but have differetn data.
    They can be different service swithin out company that provide different functionality, or they can be external services owned by different companies that partner with us.
    The core principle of the scatter getter pattern is that each reqeust sent to a different worker is entierly independent of the other reqeusts sent to the other workers
    Due to this process we can perform a large number of request scompletly in paralell, which has the potential to provide a large amaount of information in a constant time that is independednt of the number of workers.
    ...
    Each worker can become unreachable or unavailable at any given moment due to issues in the network or infrastrcture.
