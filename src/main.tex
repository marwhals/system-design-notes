...
%    Three important quality attributes when desigining a system
    ...
    \section{Scalability}
    Load and traffic patterns never stay the same. Seasonal, time etc.
    Scalability is a measure of a systems abilty to handle a growing amount of work in an easy and cost effective way by adding resources to the system
    Optimistic scenario is linear scalabilty but in practice this is hard to acheieve.
    Three ways to scale
    \begin{List}
        \item Horizontal scaling - Add more units of the resources that we have like multiple computers.
        Spread the load.
        No limit to scala. Can easily add an remove systems as required.
        Provides fault tolerance and high availablity straight away.
        ...
        This impacts team scalabilty and engineering velocity.
    \end{List}
    One solution is to seprate code into seperate services which theier own code base and tech stack, release schedule etc.

    These scalabilty solutions are orthogonal to each other.


    \section{Availability}
    Bad availabilty - Business loses money, bad things happen on important systems.
    Availability is either the fraction of time the probability that our service is operationally funtional and accessible to the user.
    That time that out system is operationally functional and acessible to the user is often referred to as the up time of the system.
    The time out sytem is unavailble is referred to as downtime.
    Availabilty is measured as a percentage representing the ration between the up time and the enture time out system is running.
    This is the totoal sum of uptime and downtime of the system.
    Availabillty is measured in percentages.
    ...
    Availabilty of a system is the mtbf divided by the sum of mtbf and mttr.
    Use formula to estimate availabilty.
%    Add equation
    Note: If we mimimise the average time to detect and recover from a failure theortically all the wy to 0 we can essentially achieve 100\%
    availabilty regardless of the average time between failures.
    This is not practical but detectability and fast recovery has a positive impact on availabilty.
    ...
    This allows for horizontal scalability but also coordingation between all the replicas since they are taking requestis.
    ...
    Lose abilty to scale out system, all the requests go to one machine.
    ...
    In either strategy if monitorign service does not hear from the for a predefined duration of time, it can assume that the server is no longer avaiable.
    Can lead to false positives.
    Doesn't need to be to be perfect, as long as their are not false negatives.
    False negatives mean that the moinitorign service failed since it didn't detect it.

    Monitoring service can be more complex
    Can mointor for certain conditions, can collect data like number of exceptions, latency etc.
    ...
    Perform a rollback. i.e go to a previous version that was stable and correct. Useful for deploying updates.
    ...
    Includes penalities if those agreements are breached.

    Example penalities include: Refunds and service credits.

    Usually exists for external paying users but sometimes for free users.
    Ocassionally for internal users but the penalties are minor if they exists.
    ...
    \subsection{SLI}
    Service level indicator - A quantative measure of a SLO using a monitoring system. Logs could be used to calculate this.
    Once calculated this can be compared to the SL Objectives.

    \subsection{}
    SLA are usually created by business and legal teams.
    SLOs are defined by the acrchitects and software engineers as well as the indicators.

    \paragraph{Considerations}
    Think about the metrics the user cares about. Dont measure everythign.
    ...
    Important for external SLAs especially to avoid penalities.
    Interanlly these goals can be more agreesive.
    ...
    This plan could include automatic alerts, automatic failovers, rollbacks, restarts, etc and handbooks for certain situations.
