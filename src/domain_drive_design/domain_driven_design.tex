...
Challenge with building Sagas is that they are complicated, but there are frameworks for it (Reactive Spceification and WebFlux).
...
The idea is that since each microsedrvice has its own data base, if failures are not addressed it will lead to an inconsistent state of data across the mlutiple databases owned by the microservices.
...
- Then there are mlutiple frameworks in different languages. Spring can be used.
...
- Even with those weaknessses, command orchestrators are much simpler to work with since there is a central component that defines the flow of the services calls.
...
Aspect to consider - Sychronous vs Asynchronous calls.

Service in the Saga may expose commands by way of APIs. Example, REST APIs and these Rest APIs may be invoked by way of an orchestraot in a synchornous fashion.
This is feasible...but asynchornous mechanisms such as messaging is the preferred way to fo.
...
The idea behind this uniqe identity is that it helps with event sourcing and the services in the saga may use it to identify duplicate transactions.
...
Idempotence is the property of certain operations in mathematics and computer science (hahahahahhaha) where they can be applied multiple times withouth changing the result beyond the initial applications.
...
When a transaction is recieved, the service will check if the transaction ID has already been processed.
...
Otherwise data across the microservices will get into inconsistent staet in order to build a capability for recovering from failed compensating transactions.
...
%TODO Potentially already done. Consider skipping for now
