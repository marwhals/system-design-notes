...
%    Add equation
    ...
    \paragraph{Some issues with flexible schemas}
    ...
    \subparagraph{Third Type}
    Third Type - A graph database, an extension of a document store but with additional capabilities to traverse, link and analyse records more efficiently.
    ...
    \subsubsection{Technique one - Indexing}
    ...
    \subsubsection{Database replication - single database, single point of failure.}
...
%TODO where does pine cone and vector data bases fit in...........
    ...
    --- Allows each developer to load their code up in their IDE much faster
    --- Building each microservice also becomes much quicker because the size of each binary is greatly reduced.
    --- Testing and reasoning about each service in isolation becomes much easier because there is much less logic to understand run and test.
...
%   TODO EXAMPLE? Online Store
    ...
    Note: There is no one right way to set the boundaries of the microservices.
    ...
    Added latency. Sending a network request to another service, parsing the reqeust, querying the database, sending it back and parsing it back defintely adds a performance overhead compared to a simple query agains the database directly.
    ...
    **** THis is very important for situations where strict consistency is important, data duplicate should be avoided.
    ...
    One team maintains a single frontend while the backend uses microservics. A single team is currently looking after the frontend.
    ...
    **Can also delegate authorization and TLS termination to the API gateway.
    ...
    %TODO consider skipping or going through for thoroughness / refresher / revisit after coding in Kafka
